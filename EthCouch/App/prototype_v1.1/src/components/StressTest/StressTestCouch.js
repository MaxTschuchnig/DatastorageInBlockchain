...
class StressTestCouch extends Component {
    constructor(props) {
        super(props);
        ...
        this.db = new PouchDB('localTickets');
        this.remote = new PouchDB('http://127.0.0.1:5984/tickets');

        /* TODO: Enable to do this test again
        this.db.sync(this.remote, {
            live: true,
            retry: true
        }).on('paused', info => {
            console.log('Synchronisation paused');
            this.stopTimer();
        }).on('active', info => {
            console.log('Synchronisation resumed');
    ...
    getDataEntries(amount) {
        ...
        for (let i = 0; i < amount; i += 1000) {
            if (i+1000 < amount) {
                this.db.bulkDocs(items.slice(i, i+1000));
            }
            else {
    ...
    render() {
        return (
            <div>
                Stress Test Couch
                <Button onClick={() => this.getDataEntries(this.state.parent.state.amount)}>Start Stress-Test Couch</Button>
                {this.state.time}
            </div>
        );
    }
...
export default StressTestCouch;