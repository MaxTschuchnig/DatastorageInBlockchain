...
class EditComponent extends Component {
    ...
    doAll() {
        ...
        tempTicket._id = this.getHash(tempTicket);
    ...
    incrementTicketId() {
        let tempTicket = this.state.ticket;
        tempTicket._seq = tempTicket._seq + 1;
        this.setState({
            ticket: tempTicket
        });
    }

    /* Not used in 1.1 anymore, delete ?
    getSequenceNbr() {
    ...
    add_item_bc() {
        ...
        this.web3.myAdmin.unlock(this.state.parent.state.user_address, 'maxi', 30)
            .then(result => {
                ...
                }, [this.convertToHex(item._task), '0x' + item._id, item._editor, item._seq]);
